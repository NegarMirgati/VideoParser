...
import sys, os, cv2, math, glob, csv
import numpy as np
...
class FrameParser:
    ...
    def GetFrameDescription(
        self, frameNumber, frame, sprites, overlapThreshold=4, minDist=4
    ):
        columns = []
        ...
        for sprite in sprites:
            ...
            result = cv2.matchTemplate(
                sprite[0], frame, cv2.TM_CCOEFF_NORMED
            )  # Change cv2.TM_CCOEFF_NORMED for more accurate but slower results
            ...
            for pt in maxVals:
                ...
                for c in columns:
                    ...
                    if addIt and not c in toRemove:
                        if (
                            abs(int(c[2]) - int(column[2])) <= minDist
                            and abs(int(c[3]) - int(column[3])) <= minDist
                        ):
                            c6 = float(c[6])
...
def MaxLocalVals(result, width, height, threshold):
    ...
    for pt in zip(*loc[::-1]):
        ...
        for pt2 in locPoints:
            if maxVal < result[pt2[1]][pt2[0]] and (
                abs(pt2[0] - pt[0]) < width / 2.0 and abs(pt2[1] - pt[1]) < height / 2.0
            ):  # Is this point already in locPoints and in range and is it better?
                maxPnt = pt2
            elif maxPnt == pt and (
                abs(pt2[0] - pt[0]) < width / 2.0 and abs(pt2[1] - pt[1]) < height / 2.0
            ):  # Inverse, Does this point beat out something in locPoints?
                toRemove.append(pt2)
...
def OverlapAmount(column1, column2, printIt=False):
    ...
    SI = max(0, max(r1[0], r2[0]) - min(l1[0], l2[0])) * max(
        0, max(r1[1], r2[1]) - min(l1[1], l2[1])
    )
...
if __name__ == "__main__":
    ...
    os.system(
        ...
        + folder
        + "image-%08d.png"
    )  # " -vf scale="+widthOfFrame+":"+heightOfFrame+
    ...
    for filename in glob.glob(directory + "*.png"):
        ...
        sprite_gray = cv2.cvtColor(
            img_rgb, cv2.COLOR_BGR2GRAY
        )  # Gray is faster to match
    ...
    target = open(folder + "frameDescriptions.csv", "w")
