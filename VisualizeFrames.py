...
if __name__ == "__main__":
    ...
    for row in reader:
        if readLine:
            ...
            objName = str(row[1])
            gameplayVals[currFrame].append(
                FrameObject(objName, int(row[2]), int(row[3]), int(row[4]), int(row[5]))
            )
            prevFrame = currFrame
    ...
    for translatedFrame in gameplayVals.keys():
        ...
        img = Image.new(
            "RGB", (int(maxX), int(maxY)), "rgb(91, 147, 251)"
        )  # CHANGE THIS TO CHANGE THE DEFAULT BACKGROUND COLOR
        pixels = img.load()
